...
def configure_ngrok():
    ngrok_token = "2pGjGxy5Cg0SnCW6G28mGkF1Qja_7PWHc6XYELvWVnGMBwFWu"
    try:
...
def update_soul_txt(public_url):
    try:
        with open("binder4.txt", "w") as file:
            file.write(public_url)
        logging.info(f"New ngrok link saved in binder4.txt")
    except Exception as e:
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("binder4.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        logging.info("Updated binder4.txt on VPS successfully.")
    except Exception as e:
        logging.error(f"Failed to update binder4.txt on VPS: {str(e)}")
...
def run_flask_app():
    ...
    def bgmi():
        ...
        command = f"./Spike {ip} {port} {duration} 5 900"
